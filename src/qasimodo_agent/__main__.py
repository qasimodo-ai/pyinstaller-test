...
def ensure_chromium_installed():
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-m", "playwright", "install", "chromium"], check=True, capture_output=True, text=True
        )
...
async def main(chromium_path):
    agent = Agent(
        ...
        browser=Browser(headless=True, chromium_sandbox=True, executable_path=chromium_path),
    )
    await agent.run()
...
def health_check():
    ...
    if getattr(sys, "frozen", False):
        print("[OK] Running as PyInstaller bundle")
        print(f"     Bundle dir: {sys._MEIPASS}")
    else:
        print("[OK] Running from source")
    ...
    if chromium_path:
        ...
        print("[WARN] Chromium not bundled (checking environment)")
        env_chromium = os.environ.get("CHROMIUM_PATH")
        if env_chromium:
            print(f"       CHROMIUM_PATH: {env_chromium}")
        else:
            print("       No CHROMIUM_PATH set")
